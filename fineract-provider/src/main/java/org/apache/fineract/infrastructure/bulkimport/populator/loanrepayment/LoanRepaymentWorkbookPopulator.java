	...
	private void setNames(Sheet worksheet) {
		...
		for (int i = 0; i < allloans.size(); i++) {
			if (!clientId.equals(allloans.get(i).getClientId().toString())) {
				...
				clientName = allloans.get(i).getClientName()+"_"+clientId;
		...
		for (int j = 0; j < clientsWithActiveLoans.size(); j++) {
			Name name = loanRepaymentWorkbook.createName();
			String excelName="Account_" + clientsWithActiveLoans.get(j).replaceAll(" ", "_") + "_" + clientIdsWithActiveLoans.get(j) + "_";
